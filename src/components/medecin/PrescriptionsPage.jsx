...
import { getPrescriptions, creerPrescription, envoyerPharmacie } from '../../api/medecinAPI';
...
const EMPTY_MED = () => ({ nom: '', posologie: '', duree: '', quantite: '' });
...
const normalizePrescription = (prescription) => ({
  ...
  medecin: prescription.medecin?.user
    ? `Dr. ${prescription.medecin.user.prenom} ${prescription.medecin.user.nom}`
    : (typeof prescription.medecin === 'string' ? prescription.medecin : ''),
...
export default function PrescriptionsPage() {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        setPrescriptions(normalizeCollection(data).map(normalizePrescription));
      } catch {
        setPrescriptions([]);
      } finally {
        setLoading(false);
  ...
  async function handleEnvoyer(id) {
    try {
      await envoyerPharmacie(id);
    } catch (error) {
      showToast(error?.response?.data?.message || 'Envoi impossible', 'error');
      return;
    }
    setPrescriptions(prev => prev.map(p => p.id === id ? { ...p, envoye_pharmacie: true } : p));
  ...
  function addMed() {
    setForm(f => ({ ...f, medicaments: [...f.medicaments, EMPTY_MED()] }));
  }
  ...
  async function handleCreate(e) {
    ...
    setSubmitting(true);
    let created = false;
    const nameParts = form.patient_nom.trim().split(' ');
    const payload = {
      patient: { prenom: nameParts[0] || '', nom: nameParts.slice(1).join(' ') || '' },
      medecin: 'Dr. Cheikh Tijan Sy',
      date_emission: form.date_emission,
      date_expiration: form.date_expiration,
      medicaments: form.medicaments,
      statut: 'active',
      envoye_pharmacie: false,
    };
    try {
      ...
      if (created) {
        setShowCreate(false);
        setForm({ patient_nom: '', date_emission: '', date_expiration: '', medicaments: [EMPTY_MED()] });
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 14, marginBottom: 20 }}>
                <div style={{ gridColumn: '1 / -1' }}>
                  ...
                  <div style={inputWrap}>
                    ...
                    <input style={inputStyle} placeholder="Prénom Nom" value={form.patient_nom} onChange={e => setForm(f => ({ ...f, patient_nom: e.target.value }))} required />
                  </div>
                </div>
                <div>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                  {form.medicaments.map((m, idx) => (
                    <div key={idx} style={medFormRow}>
                      <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1fr 80px', gap: 10, flex: 1 }}>
                        <div style={inputWrap}>
